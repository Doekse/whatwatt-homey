...
const { parseSSE, toAsyncIterable } = require('./utils');
...
class WhatwattAPI {
  ...
  async _request(method, path, body = undefined) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (this.device && responseBody) {
          this.device.error(`whatwatt API Error Body: ${responseBody.slice(0, 500)}`);
        }
        const err = new Error(`HTTP ${response.status} ${response.statusText}`);
        err.response = { status: response.status, statusText: response.statusText, data: responseBody };
        throw err;
      }
  ...
  /**
   * Fetches a single live data event from the SSE endpoint.
   *
   * Firmware 2.x gates /api/v1/report behind Plus, while /api/v1/live remains
   * available for real-time measurements. This method opens the stream, returns
   * the first live event, and closes the connection immediately.
   *
   * @param {number} [timeout=this.timeout] - Snapshot timeout in milliseconds
   * @returns {Promise<Object>} Parsed live data payload
   */
  async getLiveDataSnapshot(timeout = this.timeout) {
    await this._ensureAuth();
    ...
    if (this.device) {
      this.device.log(`whatwatt API Request: GET ${path} (snapshot)`);
    }
    ...
    try {
      ...
      if (this.device) {
        this.device.log(`whatwatt API Response: ${response.status} ${response.statusText} (${path} snapshot)`);
      }
      ...
      throw new Error('Live data stream ended before receiving a live event');
    } catch (error) {
      if (error.name === 'AbortError') {
        const abortErr = new Error('Request timeout');
        abortErr.request = true;
        throw this._handleError('Failed to get live data snapshot', abortErr);
      }
      throw this._handleError('Failed to get live data snapshot', error);
    } finally {
      clearTimeout(timeoutId);
      controller.abort();
    }
  ...
  _handleError(message, error) {
    if (error.response) {
      ...
      switch (status) {
        ...
        case 404:
          if (typeof error.response.data === 'string' && /license required/i.test(error.response.data)) {
            const licenseError = new Error(`${message}: Plus license required`);
            licenseError.isLicenseRequired = true;
            licenseError.statusCode = 404;
            return licenseError;
          }
