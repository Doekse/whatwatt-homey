...
class whatwattEventStream {
   ...
   * Accepts connection parameters and callback functions for handling events.
   * 
  ...
  /**
   * Starts the SSE connection to the whatwatt device.
   * Sets up event listeners and handles authentication if provided.
   */
  start() {
    if (this.eventSource) {
    ...
    try {
      ...
      // Set up EventSource headers for SSE protocol
   ...
   * Verifies both internal state and EventSource readyState.
   * 
   ...
   * Prevents overwhelming the device with rapid reconnection attempts.
   */
  _scheduleReconnect() {
  ...
  /**
   * Resets the reconnection counter to zero.
   * Called when connection settings change to allow fresh reconnection attempts.
   */
  ...
  /**
   * Updates connection settings and restarts the stream if needed.
